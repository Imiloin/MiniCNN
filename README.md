...
大部分结果与参考结果完全一致。少数结果存在误差，这是重量化时的策略存在区别，本设计中采用了乘法 + 移位的重量化方法，用 $M = 2^{-n}M_0$ 的方式计算重量化因子，这导致少部分输出的 INT8 定点数与参考实现有 $\pm 1$ 的差别，经过 Sigmoid 激活后便出现了表中的不一致数据。但这样的误差不会影响最终的分类结果，在可接受范围内。

## A little more

本课程项目的体验十分糟糕，题目难度很不合理，提供的文档中许多细节都未表述清楚。例如在定点数发生溢出时，有些层使用了截断的方式，而有些层使用了饱和的方式，这些细节在文档中完全没有提及，导致在实现过程中需要不断地进行猜测和调试，浪费了大量的时间和精力。

不仅如此，做出来的这种 CNN 架构早已过时，实际价值约等于零。\[1\] 中也指出，在两个关键词的情况下该网络的准确率仅有 94.6%，这对于二分类的语音识别任务来说是一个灾难级的结果。

同时，这门课的教师也完全不上心，几乎提供不了指导。
