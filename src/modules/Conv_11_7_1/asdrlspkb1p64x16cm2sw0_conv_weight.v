...
module asdrlspkb1p64x16cm2sw0_conv_weight ( Q, ADR, D, WE, ME, CLK, TEST1, RM, RME);

// Input/Output Ports Declaration
output  [7*11-1:0] Q; //读数据，输出要读的信号
input  [4:0] ADR;          //地址
input  [7*11-1:0] D;  //写数据，输入要写的信号
input WE;                  //写使能信号
input ME;                  //读使能信号
input CLK;
input TEST1;
...
generic_behav_asdrlspkb1p64x16cm2sw0_conv_weight #( MES_ALL) uut ( .Q(Q_tmp), .ADR(ADR), .D(D), .WE(WE), .ME(ME), .CLK(CLK) );
...
module generic_behav_asdrlspkb1p64x16cm2sw0_conv_weight (  Q, ADR, D, WE, ME, CLK );

parameter MES_ALL = "ON";
parameter words = 32, bits = 7*11, addrbits = 5;
...
wire [1:0] ADR_valid;
reg [7*11-1:0] mem_core_array [0:31];
...
assign ADR_valid = (( ^ADR === 1'bx ) ? 2'b01 : ( ( ADR > 5'b11111 ) ? 2'b10 : 2'b00 ));
