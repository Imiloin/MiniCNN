...
module asdrlspkb1p64x16cm2sw0_dwconv_bias ( Q, ADR, D, WE, ME, CLK, TEST1, RM, RME);

// Input/Output Ports Declaration
output  [31:0] Q; //读数据，输出要读的信号
input  [4:0] ADR;          //地址
input  [31:0] D;  //写数据，输入要写的信号
input WE;                  //写使能信号
input ME;                  //读使能信号
input CLK;
input TEST1;
...
generic_behav_asdrlspkb1p64x16cm2sw0_dwconv_bias #( MES_ALL) uut ( .Q(Q_tmp), .ADR(ADR), .D(D), .WE(WE), .ME(ME), .CLK(CLK) );
...
module generic_behav_asdrlspkb1p64x16cm2sw0_dwconv_bias (  Q, ADR, D, WE, ME, CLK );

parameter MES_ALL = "ON";
parameter words = 32, bits = 32, addrbits = 5;
